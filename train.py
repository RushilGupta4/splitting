...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train DDPM for a 2D Gaussian-mixture distribution"
    )
    parser.add_argument("--T", type=int, default=1000, help="Number of diffusion steps")
    parser.add_argument(
        "--num_samples", type=int, default=100_000, help="Number of training samples"
    )
    parser.add_argument("--batch_size", type=int, default=10_000, help="Batch size")
    parser.add_argument(
    ...
    parser.add_argument(
        "--num_blocks", type=int, default=4, help="Number of ResNet blocks"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=20_000,
        help="Number of samples used for final marginal plot",
    )
    parser.add_argument(
        "--num_plot_bins",
        type=int,
        default=100,
        help="Number of bins used in final marginal histogram",
    )
    parser.add_argument(
        "--marginal_plot_path",
        type=str,
        default=None,
        help="Optional output path for the final marginal plot",
    )
    parser.add_argument(
...
class DDPM:
    ...
    def p_sample_step(self, model, x_t, t):
        ...
        if t > 0:
            # DDPM uses posterior variance beta_tilde_t, not beta_t.
...
def create_dataloader(num_samples, batch_size, device):
    """Create a DataLoader with normalized samples from the 2D Gaussian mixture."""
...
def train(args):
    target_spec = get_target_distribution_spec()
    print("Training DDPM for the 2D Gaussian-mixture target")
    ...
    if marginal_plot_path is None:
        marginal_plot_path = os.path.join(args.checkpoint_dir, "marginals_final.png")
