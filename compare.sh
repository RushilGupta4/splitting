...
do
    ...
    do
        (
            ...
            uv run python src/compare.py --runner "$runner" --config "$config_name" --output_dir "$output_dir" --device "$device" --n_parallel $N_PARALLEL --n_runs $N_RUNS --no_compile $debug_flag || exit 1
